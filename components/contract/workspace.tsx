    ...
    <>
      ...
      <div className="border-b border-line bg-white/60">
        <div className="mx-auto max-w-6xl px-6 pt-8 pb-0">
          <nav className="flex items-center gap-1.5 text-[12.5px] text-muted">
            <Link href="/contracts" className="transition-colors hover:text-plum-700">
              Contracts
            </Link>
            <ChevronRight width={13} height={13} className="text-line-strong" />
            <span className="truncate text-plum-700">{contract.title}</span>
          </nav>
          ...
          <div className="mt-7 flex gap-1 overflow-x-auto">
            {tabs.map((t) => (
              ...
              >
                {t.label}
                {t.badge ? (
                  <span className="rounded-full bg-clay-50 px-1.5 py-0.5 text-[10.5px] font-semibold text-clay-700">
                    {t.badge}
                  </span>
                ) : null}
                <span
                  className={cn(
                    ...
                    tab === t.key ? "bg-plum-700" : "bg-transparent",
                  )}
                />
              </button>
            ))}
      ...
      {drawer && selected ? (
        <div className="fixed inset-0 z-50 flex items-end lg:hidden">
          <button
            className="absolute inset-0 bg-plum-900/25 backdrop-blur-[2px]"
